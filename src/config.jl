...
"""
function chk_dict()
    @assert get_b("solver") in
            ("MaxEnt", "BarRat", "NevanAC", "StochAC", "StochSK", "StochOM", "StochPX")
    @assert get_b("ktype") in ("fermi", "boson", "bsymm")
    @assert get_b("mtype") in (
        "flat",
        "gauss",
        "1gauss",
        "2gauss",
    ...
    @assert get_b("grid") in
            ("ftime", "fpart", "btime", "bpart", "ffreq", "ffrag", "bfreq", "bfrag")
