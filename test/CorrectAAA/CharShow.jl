...
σ = [0.2, 0.8]
peak = [1.0, 0.3]
A = continous_spectral_density(μ, σ, peak) # 连续谱密度
β = 10.0
N = 20 # 输入数据点数
output_bound = 5.0 # 输出数据范围
output_number = 801 # 输出数据点数
noise = 1e-6

mesh, reA = reconstruct_spectral_density(A, β, N, output_bound, output_number, noise)

plot(mesh, reA, label = "reconstructed", legend = :bottomright)
plot!(mesh, A.(mesh), label = "original", legend = :bottomright)
